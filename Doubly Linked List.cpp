...
public:
    ...
    void menu(void)
    {
        cout<<" DOUBLY LINKED LIST :: MAIN MENU\n"
    ...
    void deletepos(int val)
    {
        ...
        if(count<val && temp->next==NULL)
        {
            cout<<"ERROR : No node found in given position."<<endl;
            return;
        }
        if(count==val-1 && temp->next->next==NULL)
        {
            deleteend();
            return;
        }
    ...
    void display()
    {
        ...
        do
        {
            cout<<setw(10)<<temp->prev<<setw(7)<<temp->data<<setw(13)<<temp->next<<endl;
            temp=temp->next;
        }while(temp!=NULL);
    }
};
